...
def calculate_all_scores_revised(adj_sparse, train_test_split, random_state=0, tf_dtype=tf.float32):
    ...
    # Use dot product
    tf.set_random_seed(random_state) # Consistent GAE training
    gae_results = gae_scores(adj_sparse, train_test_split, None,LEARNING_RATE, EPOCHS, HIDDEN1_DIM, HIDDEN2_DIM, DROPOUT, \
                             "dot-product",0,dtype=tf.float16)
    lp_scores['gae'] = gae_results
