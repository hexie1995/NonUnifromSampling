...
def gen_topol_feats(num_nodes_orig, A, edge_s): 
    ...
    for ee in range(len(edge_s)):
        adma_adar_obj = nx.adamic_adar_index(G, [edge_s[ee][0], edge_s[ee][1]])
        try:
            for uu,vv,jj in adam_adar_obj:
                adam_adar_edges.append([uu,vv,jj])
        except:
            adam_adar_edges.append([edge_s[ee][0], edge_s[ee][1], 0])
...
def topol_stacking(edges_orig): 
    ...
    test_name = "edge_set"+ str(i) +"_test"
            
    train_name = "edge_set"+ str(i) +"_train"
...
def topol_stacking_for_sampling_old(DATA, net, count, samp): 
    ...
    test_name = "edge_set"+ str(count) +"_test"
    train_name = "edge_set"+ str(count) +"_train"
        
    
    orig_edge = DATA["edges_id"][net]
    hold_edge = DATA[test_name][net]
    train_edge = DATA[train_name][net]
    ...
    #### load the training edges to a matrix
    num_nodes = int(np.max(train_edge)) + 1
    row = np.array(train_edge)[:,0]
    col = np.array(train_edge)[:,1]

    data_aux = np.ones(len(row))
    A_train = csr_matrix((data_aux,(row,col)),shape=(num_nodes,num_nodes))
    A_train = sparse.triu(A_train,1) + sparse.triu(A_train,1).transpose()
    A_train[A_train>0] = 1 
    A_train = A_train.todense()
    
    
    num_nodes = int(np.max(hold_edge)) + 1
    row = np.array(hold_edge)[:,0]
    col = np.array(hold_edge)[:,1]

    data_aux = np.ones(len(row))
    A_hold = csr_matrix((data_aux,(row,col)),shape=(num_nodes,num_nodes))
    A_hold = sparse.triu(A_hold,1) + sparse.triu(A_hold,1).transpose()
    A_hold[A_hold>0] = 1 
    A_hold = A_hold.todense()
    ...
    edge_t_tr = DATA[samp+"_"+str(count) +"_"+ "t" + "_" + "train" + "_10000"][net]
    edge_f_tr = DATA[samp+"_"+str(count) +"_"+ "f" + "_" + "train" + "_10000"][net]
    
    df_f_tr = gen_topol_feats(num_nodes_orig, A_train, edge_f_tr)
    df_t_tr = gen_topol_feats(num_nodes_orig, A_train, edge_t_tr)
    
    edge_t_ho = DATA[samp+"_"+str(count) +"_"+ "t" + "_" + "test" + "_10000"][net]
    edge_f_ho = DATA[samp+"_"+str(count) +"_"+ "f" + "_" + "test" + "_10000"][net]
