...
def process_time(samp):
    ...
    A_ho, A_tr, A_test, A_orig, A_val = gen_tr_ho_networks(A_orig, 0.8, 0.8, 1) 
    train_tr, ne_ho, e_diff, ne_orig = sample_true_false_edges(A_orig, A_tr, A_ho, 1)
    ...
    with open('timed.txt', 'a') as fd:
        fd.write('\n')
        fd.write(str(samp))
        fd.write(str(end-start))

netlist = list(range(0,1)) 

with Pool(len(sampling_methods)) as p:
    print(p.map(process_time, sampling_methods))
