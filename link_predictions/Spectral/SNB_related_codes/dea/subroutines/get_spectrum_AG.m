function [ V,D,V_clustering ] = get_spectrum_AG( A,DEA,mode,numvec,numvec_eigs )
%GET_SPECTRUM compute spectrum of various matrices.
    ...
    if(numvec>0)
        ...
        elseif(mode==5) % 
            ...
            F=[sparse(N,N) D-I;-I A];
            [V, D] = eig(full(F));
            V_clustering=V(length(A)+1:2*length(A),:);
        else 
            printf('wrong mode\n');  return;
        end   
    ...
    D = D(ind);
    V = V(:,ind);
    V_clustering = V_clustering(:,ind);
    if(numvec>0)
        D=D(1:numvec);
        V=V(:,1:numvec);
        V_clustering = V_clustering(:,1:numvec);
    end
end
